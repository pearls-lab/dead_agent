...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        if self.replay_buffer is None:
            ...
            # Just having the aux be a copy of the primary replay buffer for now
            self.aux_replay_buffer = self.replay_buffer_class(
                self.buffer_size,
                self.observation_space,
                self.action_space,
                device=self.device,
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            rollout = self.collect_rollouts(
                ...
                replay_buffer=self.replay_buffer,
                aux_replay_buffer = self.aux_replay_buffer,
    ...
    def _store_transition_aux_prep(
        ...
        """
        Does all pre-processing up to the point of actually putting into the buffer

        Mainly for the auxilary buffer, where we want to only selectively place rollouts into that.
    ...
    def collect_rollouts(
        ...
        replay_buffer: ReplayBuffer,
        aux_replay_buffer: ReplayBuffer,
        action_noise: Optional[ActionNoise] = None,
